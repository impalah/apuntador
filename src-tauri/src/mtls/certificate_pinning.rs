...
impl CertificatePinner {
    ...
    pub fn verify_chain(&self, certs: &[Certificate]) -> Result<(), String> {
...
impl PinningVerifier {
    pub fn new(pinner: CertificatePinner) -> Result<Self, TlsError> {
        ...
        Ok(Self {
            ...
            webpki_verifier: Arc::new(webpki_verifier),
        })
    }
}
...
mod tests {
    ...
    fn test_fingerprint_calculation() {
        let test_cert = Certificate(vec![1, 2, 3, 4, 5]);
    ...
    fn test_verify_chain_with_matching_pin() {
        let cert = Certificate(vec![1, 2, 3, 4, 5]);
