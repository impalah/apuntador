...
public class ApuntadorAutoEnrollmentPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    /**
     * Envía petición de enrollment con Certificate Pinning
     * NO usa certificado de cliente (aún no lo tenemos)
     * SÍ valida el certificado del servidor
     */
    private func sendEnrollmentRequestWithPinning(url: String, data: [String: Any]) async throws -> (Data, HTTPURLResponse) {
        guard let requestURL = URL(string: url) else {
            throw EnrollmentError.invalidResponse
        }
        
        // Crear MTLSHTTPClient sin configurar certificado de cliente
        let httpClient = MTLSHTTPClient()
        
        // Configurar URLSession con certificate pinning pero sin certificado de cliente
        let configuration = URLSessionConfiguration.default
        configuration.timeoutIntervalForRequest = 30
        let session = URLSession(
            configuration: configuration,
            delegate: httpClient,  // El delegate hace certificate pinning
            delegateQueue: nil
        )
    ...
    /**
     * Envía petición de enrollment SIN Certificate Pinning (deprecated)
