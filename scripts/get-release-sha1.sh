...
if [ -z "$STORE_PASS" ]; then
    echo "❌ Error: No se pudo leer storePassword de key.properties"
    exit 1
fi
...
if [ -z "$SHA1" ]; then
    echo "❌ Error: No se pudo obtener el SHA-1"
...
echo ""
echo "⚠️  Recuerda también agregar el SHA-1 del debug keystore"
