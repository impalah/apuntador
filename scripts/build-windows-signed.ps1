...
if (!(Test-Path $CertificatePath)) {
    Write-Host "❌ Certificado no encontrado: $CertificatePath" -ForegroundColor Red
...
try {
    ...
    if ($cert) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "⚠️  Los usuarios verán un aviso de seguridad pero será menos severo" -ForegroundColor Yellow
            
        } else {
            Write-Host "❌ Error en la compilación de Tauri" -ForegroundColor Red
            exit 1
        }
