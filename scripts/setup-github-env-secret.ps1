...
if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
    Write-Host "[ERROR] Error: GitHub CLI (gh) no está instalado" -ForegroundColor Red
    Write-Host ""
    Write-Host "[DOWNLOAD] Instalar GitHub CLI:" -ForegroundColor Yellow
...
if ($confirmation -notmatch '^[SsYy]$') {
    Write-Host "[ERROR] Operación cancelada" -ForegroundColor Red
    exit 1
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "[TIP] Alternativa: Configura el secret manualmente en:" -ForegroundColor Yellow
