...
if ! command -v xcodebuild &> /dev/null; then
    echo "[ERROR] Error: Xcode is not installed or not in PATH"
    exit 1
fi
...
if [ ! -d "dist" ]; then
    echo "[ERROR] Error: Web build failed - dist directory not found"
    exit 1
fi
...
if [ ! -f "ios/App/App/Assets.xcassets/AppIcon.appiconset/Contents.json" ]; then
    echo "[WARNING]  Warning: App icons may not be properly configured"
